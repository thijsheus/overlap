...
"""
def ra1(grad,ht,w,cv,cb,one):
    multiple=(abs(grad)*ht)/np.maximum(abs(w),one)
    multiplicand=1/(cv)**(0.5)
...
def ra(grad,ht,w,ct,cb,one):
    ...
    return ((1)+multiple*multiplicand)**(-1)
############
def raalt(grad,ht,w,ct,cb,one):
   term1=abs(grad*ht)/(np.maximum(abs(w),one)*cb)
   term2=abs(ct-cb)/cb
    ...
    if file == bomexd:
        for file1 in bomexfilenames:
            ...
            for t in range(time_t.size):
                ...
                #plt.plot(ra1(gradint(ztnew),ht,wnew2(ztnew),ct,cb,one),gradint(ztnew),'-')
                #plt.title('grad vs. ratio')
                ##plt.hist2d(gradint(ztnew),overlap_ratio,bins=bins,cmin=0.5)
                ##############################################################
                plt.hist2d(rashear(gradint(ztnew),ht,wnew2(ztnew),ct,cb,one),gradint(ztnew),bins=bins,cmin=0.5)
                plt.title('shear vs. ratio')
                #plt.hist2d(rashear(gradint(ztnew),ht,wnew2(ztnew),ct,cb,one),gradint(ztnew)*ht,bins=bins,cmin=0.5)
                #plt.title('shear*ht vs. ratio')
                #plt.hist2d(raanvil(gradint(ztnew),ht,wnew2(ztnew),ct,cb,one),abs(ct-cb),bins=bins,cmin=0.5)
                #plt.title('ct-cb vs. ratio')
                #plt.hist2d(ra(gradint(ztnew),ht,wnew2(ztnew),ct,cb,one),ht,bins=bins,cmin=0.5)
                #plt.title('ht vs. ratio')
                ##plt.plot(ra(gradint(ztnew),ht,wnew2(ztnew),ct,cb,one),ht,'o')
                #plt.hist2d(overlap_ratio,ht,bins=bins,cmin=0.5)
                
                #z=np.polyfit(overlap_ratio,ht,1)
                #plt.plot(overlap_ratio,z[0]*overlap_ratio +z[1])
                ###############################################################
